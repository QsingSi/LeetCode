 ...
 */
class Solution {
    private void insertRow(TreeNode rt, int v, int d, int now) {
        if (rt == null)
            return;
        if (now == d - 1) {
            TreeNode left = rt.left;
            TreeNode right = rt.right;
            rt.left = new TreeNode(v);
            rt.left.left = left;
            rt.right = new TreeNode(v);
            rt.right.right = right;
            return;
        }
        insertRow(rt.left, v, d, now + 1);
        insertRow(rt.right, v, d, now + 1);
    }

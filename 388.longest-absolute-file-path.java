 ...
 */
class Solution {
    private int computeDirLevel(String s, int start) {
        int level = 1;
    ...
    public int lengthLongestPath(String input) {
        ...
        for (int i = 0; i < input.length();) {
            if (input.charAt(i) == '\n') {
                dirLevel = computeDirLevel(input, i + 1);
                i += dirLevel;
            } else {
                int dirLength = 0;
                ...
                i += dirLength;
                while (pathSumStack.size() > dirLevel)
                    pathSumStack.pop();
                pathSumStack.push(pathSumStack.peek() + dirLength);
                if (isFile)
                    longestPath = Math.max(longestPath, pathSumStack.peek() + pathSumStack.size() - 2);
            }
        }
        return longestPath;
    }

    ...
    public List<List<String>> findDuplicate(String[] paths) {
        ...
        for (String str : paths) {
            ...
            for (int i = 1; i < tmp.length; i++) {
                ...
                sb.append(tmp[0]).append('/').append(file.substring(0, index));
                if (map.containsKey(content)) {
                    List<String> tt = map.get(content);
                    tt.add(sb.toString());
                    map.put(content, tt);
                } else {
        ...
        List<List<String>> dummy = new ArrayList(map.values());
        for (List<String> fuck : dummy)
            if (fuck.size() >= 2)
                res.add(fuck);
        return res;
    }

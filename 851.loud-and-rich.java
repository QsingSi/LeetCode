...
class Solution {
    public int[] loudAndRich(int[][] richer, int[] quiet) {
        int[][] g = new int[quiet.length][quiet.length];
        for (int[] arr : richer)
            g[arr[1]][arr[0]] = 1;
        int[] ans = new int[quiet.length];
        Arrays.fill(ans, -1);
        for (int i = 0; i < ans.length; i++) {
            helper(g, i, ans, quiet);
        }
        return ans;
    }
    private int helper(int[][] g, int u, int[] ans, int[] quiet) {
        if (ans[u] != -1)
            return ans[u];
        ans[u] = u;
        for (int i = 0; i < g.length; i++) {
            if (g[u][i] == 1) {
                int tmp = helper(g, i, ans, quiet);
                if (quiet[tmp] < quiet[ans[u]])
                    ans[u] = tmp;
            }
        }
        return ans[u];

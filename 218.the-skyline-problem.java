...
class Solution {
    ...
    public List<int[]> getSkyline(int[][] buildings) {
        ...
        for (int[] building : buildings) {
            list.add(new Node(building[0], building[2], true));
            list.add(new Node(building[1], building[2], false));
        }
        ...
        for (Node node : list) {
            if (node.isStart) {
                if (q.isEmpty() || node.height > q.peek())
                    res.add(new int[] {node.x, node.height});
                q.offer(node.height);
            } else {
                q.remove(node.height);
                if (q.isEmpty()) res.add(new int[] {node.x, 0});
                else if (node.height > q.peek()) res.add(new int[] {node.x, q.peek()});
            }
        }
        return res;
    }

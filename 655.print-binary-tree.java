...
class Solution {
    ...
    public List<List<String>> printTree(TreeNode root) {
        ...
        helper(root, res, 0, height, 0, cols - 1);
        return res;
    }
    private void helper(TreeNode rt, List<List<String>> res, int row, int totRow, int i, int j) {
        if (row == totRow || rt == null)
            return;
        res.get(row).set((i + j) / 2, Integer.toString(rt.val));
        helper(rt.left, res, row + 1, totRow, i, (i + j) / 2 - 1);
        helper(rt.right, res, row + 1, totRow, (i + j) / 2 + 1, j);
    }
}

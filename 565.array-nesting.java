 ...
 */
class Solution {
    private int dfs(int[] nums, Set<Integer> s, int index, int[] dp) {
        if (s.contains(nums[index]))
            return s.size();
        if (dp[index] != 0)
            return dp[index];
        s.add(nums[index]);
        int cnt = dfs(nums, s, nums[index], dp);
        dp[index] = cnt;
        return cnt;
    }
    public int arrayNesting(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++)
            max = Math.max(max, dfs(nums, new HashSet<>(), i, dp));
        return max;
    }
}

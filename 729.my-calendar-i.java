...
class MyCalendar {
    ...
    TreeMap<Integer, Integer> mask;
    public MyCalendar() {
        mask = new TreeMap<>();
    }

    public boolean book(int start, int end) {
        Integer key = mask.floorKey(start);
        if (key != null && mask.get(key) > start)
            return false;
        Integer newKey = mask.ceilingKey(start);
        if (newKey != null && newKey < end)
            return false;
        mask.put(start, end);
        return true;
    }

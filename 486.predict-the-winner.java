...
class Solution {
    private int dfs(int i, int j, int[] nums, int[][] memo) {
        ...
        if (memo[i][j] > 0)
            return memo[i][j];
        int a = nums[i] + Math.min(dfs(i + 1, j - 1, nums, memo), dfs(i + 2, j, nums, memo));
        int b = nums[j] + Math.min(dfs(i, j - 2, nums, memo), dfs(i + 1, j - 1, nums, memo));
        memo[i][j] = Math.max(a, b);
        return memo[i][j];
    }
    public boolean PredictTheWinner(int[] nums) {
        int[][] memo = new int[nums.length][nums.length];
        int p1 = dfs(0, nums.length - 1, nums, memo);
        int sum = 0;
        for (int num : nums)
            sum += num;
        return 2 * p1 >= sum;

...
class Solution {
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (int i = 0; i < cpdomains.length; i++) {
            ...
            int index = tmp[1].length() - 1;
            mask.put(tmp[1], mask.getOrDefault(tmp[1], 0) + cnt);
            for (; index >= 0; index--) {
                if (tmp[1].charAt(index) == '.') {
                    String domain = tmp[1].substring(index + 1, tmp[1].length());
                    mask.put(domain, mask.getOrDefault(domain, 0) + cnt);
                }
            }
        }
